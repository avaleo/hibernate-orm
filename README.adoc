...
This is a fork of Hibernate, that was created solely for fixing https://hibernate.atlassian.net/browse/HHH-19700. It was the issue that surfaced during migration of Hibernate from 4.x to 5.x in Nexus.
The issue is fixed in 6.x. The fix was rejected by Hibernate, because version 5.x is not maintained any more.

I produced a jar with version 5.6.15.1 from /hhh-19700 branch and manually deployed it to our artifact repository.

Then https://kmddk.visualstudio.com/NEXUS/_workitems/edit/2034898 was found, and another version has (5.6.15.2) was created with the fix of the issue.

Versions used by nexus are tagged (with version number).
